![header](https://capsule-render.vercel.app/api?type=wave&color=0:FFC0CB,100:B6EADA&height=300&section=header&text=hyeryeong&fontSize=90&fontColor=ffffff)

<div align="center">

### 🌱 Frontend Developer in Progress  
React · Vue · Flutter · React Native  
**사용자 흐름을 설계하고, 화면/상태/API를 안정적으로 연결하는 프론트엔드 개발자**를 목표로 합니다.

</div>

---

## ✨ Highlights
- 사용자 경험을 기준으로 **페이지 흐름/라우팅 구조**를 설계하고 구현합니다.
- API 명세 불일치/응답 구조 혼재 상황에서 **FE 모델 표준화**를 통해 안정적인 연동을 지향합니다.
- 컴포넌트/스타일 구조를 정리하며 **리팩토링과 유지보수성 개선**에 관심이 많습니다.
- 반응형 UI·디테일·인터랙션을 챙기며 “완성도”를 끌어올리는 작업을 좋아합니다.

---

## 🎓 Bootcamp
- **KT AIVLE SCHOOL 5기**
- **SSAFY 14기**

---

## ⭐ Featured Projects
> 아래 링크는 유저 깃헙 레포/노션/시연영상으로 교체하면 완성도 급상승합니다.

### 1) ARNNECT — 취향 분석 기반 예술 전시 플랫폼
- **키워드**: 큐레이션 · 피드 · 3D 전시 홀 · QR 티켓 · 컬렉트북 · 팬레터
- **내 기여(예시)**  
  - React Router 기반 라우팅/가드 구조 정리  
  - Zustand 상태 설계 및 로그인/권한 흐름 연동  
  - API 연동 시 응답 형태/필드명 차이를 FE에서 표준화  
  - 이미지/미디어 URL 처리 로직 정리(DEV/PROD 환경 분기)
- **Tech**: React, TypeScript, Vite, React Router, Zustand, Axios, r3f/Three.js  
- **Links**: [Repo](https://github.com/YOUR-USERNAME/arnnect-fe) 

### 2) 커버메이트 — 소설 요약 + 생성형 AI 표지 생성 플랫폼
- **내 기여**: 로그인/회원가입/마이페이지, 토큰 기반 인증 흐름, UI/UX 개선
- **Tech**: React, HTML, CSS  
- **Links**: [Repo](https://github.com/YOUR-USERNAME/covermate) 

### 3) 귀농백과 — 귀농 초보를 위한 정보/커뮤니티 앱
- **내 기여**: 익명 커뮤니티/댓글 기능, UI/UX 전 페이지 디자인, 게시글/댓글 수정·삭제
- **Tech**: Flutter, Dart  
- **Links**: [Repo](https://github.com/YOUR-USERNAME/farming-encyclopedia) 

---

## 🎯 What I’m focusing on
- 상태 관리 패턴(Zustand / 비동기 데이터 관리)
- 렌더링 최적화와 사용자 체감 성능 개선
- UI 컴포넌트 구조화(재사용성/응집도/결합도)
- 타입 안정성(TypeScript 모델링)과 리팩토링

---

## 💻 Algorithm
<div align="center">

[![Solved.ac Profile](http://mazassumnida.wtf/api/v2/generate_badge?boj=gaeryeong)](https://solved.ac/gaeryeong)

</div>

---

## 🛠 Tech Stack
<div align="center">

<!-- Frontend -->
<img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=000"/>
<img src="https://img.shields.io/badge/Vue-42b883?style=for-the-badge&logo=vue.js&logoColor=fff"/>
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=fff"/>
<img src="https://img.shields.io/badge/JavaScript-FFE873?style=for-the-badge&logo=javascript&logoColor=000"/>

<br/>

<!-- Mobile -->
<img src="https://img.shields.io/badge/React%20Native-61DAFB?style=for-the-badge&logo=react&logoColor=000"/>
<img src="https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=fff"/>
<img src="https://img.shields.io/badge/Dart-0175C2?style=for-the-badge&logo=dart&logoColor=fff"/>

<br/>

<!-- Tooling / State / Router / API -->
<img src="https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=fff"/>
<img src="https://img.shields.io/badge/React%20Router-CA4245?style=for-the-badge&logo=reactrouter&logoColor=fff"/>
<img src="https://img.shields.io/badge/Zustand-000000?style=for-the-badge&logo=ghost&logoColor=fff"/>
<img src="https://img.shields.io/badge/Axios-5A29E4?style=for-the-badge&logo=axios&logoColor=fff"/>

<br/>

<!-- 3D / Collaboration -->
<img src="https://img.shields.io/badge/Three.js-000000?style=for-the-badge&logo=three.js&logoColor=fff"/>
<img src="https://img.shields.io/badge/r3f-000000?style=for-the-badge&logo=react&logoColor=fff"/>
<img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=fff"/>

</div>

---

## 📊 GitHub Stats
<div align="center">

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=hyeryeongeda&layout=compact&theme=tokyonight)

<!-- ![GitHub Stats](https://github-readme-stats.vercel.app/api?username=hyeryeongeda&show_icons=true&theme=tokyonight) -->
<!-- ![GitHub Streak](https://streak-stats.demolab.com?user=hyeryeongeda&theme=tokyonight) -->

</div>

---

## 📝 Writing (Blog)
[Blog](https://premiereneige.tistory.com/)

---

## 📬 Contact
<div align="center">

[![Email](https://img.shields.io/badge/Email-ADD8E6?style=for-the-badge&logo=gmail&logoColor=white)](mailto:gaeryeongeda@naver.com)


</div>

---

<div align="center">

_꾸준히 기록하고 성장합니다._

</div>
